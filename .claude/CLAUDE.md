...
## IMPORTANT: Codebase Intelligence Instructions for EdgeRoute
...
Last indexed: 2026-06-23 (commit 0374416). Confidence: 100%.
### Architecture
repo orchestrates edge-computing infrastructure by ingesting OCI artifacts & network configurations, reconciling state via XDP/NFT filtering, and enforcing policy through a distributed Rust-based agent.
### Entry Points
- `crates/edge-core/src/lib.rs`
- `crates/edge-store/src/lib.rs`
- `crates/edge-reconcile/src/lib.rs`
- `crates/edge-xdp/src/lib.rs`
- `crates/edge-tailscale/src/lib.rs`
- `crates/edge-nft/src/lib.rs`
- `crates/edge-oci/src/lib.rs`
- `crates/edge-linux/src/lib.rs`
### Tech Stack
**Languages:** Rust
...
| Layer | Files | Purpose |
|-------|-------|---------|
| Command Line Interface | 1 | Provides the primary user-facing entry point for interacting with the edge… |
| Project Configuration Management | 3 | Contains CI/CD pipeline definitions and template configuration files for… |
| Core Application Logic | 37 | Houses the primary business logic, state management, NFT processing, and… |

### Guided Tour (12 steps)
1. **README.md** — `README.md`
2. **main.rs** — `crates/edge-agent/src/main.rs`
3. **main.rs** — `crates/edge-cli/src/main.rs`
4. **lib.rs** — `crates/edge-core/src/lib.rs`
5. **lib.rs** — `crates/edge-xdp/src/lib.rs`
6. **lib.rs** — `crates/edge-store/src/lib.rs`
... and 6 more steps
### Hotspots (High Churn)
| File | Churn | 90d Commits | Owner |
|------|-------|-------------|-------|
| `crates/edge-store/src/lib.rs` | 95.0th %ile | 4 | Nassim Zen |
| `crates/edge-core/src/validation.rs` | 85.0th %ile | 5 | Nassim Zen |
| `crates/edge-cli/src/main.rs` | 80.0th %ile | 3 | Nassim Zen |
| `crates/edge-reconcile/src/lib.rs` | 75.0th %ile | 4 | Nassim Zen |

## Code health
Hotspot health: 3.14/10 (stable) ·
Average: 6.39/10 ·
Worst: 1.0/10 (`crates/edge-core/src/validation.rs`)

### Critical biomarkers
- `crates/edge-cli/src/main.rs` — nested complexity (handle_oracle) — impact −2.5
- `crates/edge-reconcile/src/lib.rs` — brain method (reconcile) — impact −1.0
- `crates/edge-core/src/validation.rs` — brain method (validate_mapping) — impact −0.6
